...
CUSTOMERS_CSV = os.path.join(
    "..", "..", "..", "Data", "Processed", "customers_data_cleaned.csv"
)
PRODUCTS_CSV = os.path.join(
    "..", "..", "..", "Data", "Processed", "products_data_cleaned.csv"
)
...
def load_csv_to_table(conn, csv_path, table, columns):
    ...
    conn.executemany(
        f"INSERT OR IGNORE INTO {table} ({columns_str}) VALUES ({placeholders})", rows
    )
    conn.commit()
...
def main():
    ...
    load_csv_to_table(
        conn, CUSTOMERS_CSV, "customers", [c.lower() for c in customer_cols]
    )
    # Load products
    product_cols = [
        "ProductID",
